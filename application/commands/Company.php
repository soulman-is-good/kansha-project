        ...
        try {
            ...
            if(($parse_type == 0 && $params['articule']!=='') || $parse_type == 1)
                foreach($vs as $idx=>$v) {
                    ...
                    if (empty($url) || !X3_String::create($url)->check_protocol() || !X3_Thread::create($url)->run(X3_Thread::CHECK_CONNECTION)){//(($rco=X3_Thread::create($url)->run(X3_Thread::RESPONSE_CODE))<309 && $rco>100)) {
                        $url = '';
                    }
                    $item = NULL;
                    if($v[1]==0){
