...
$models = X3::db()->query("SELECT DISTINCT ci.id id, ci.item_id, ci.company_id, ci.price, ci.url, dc.delivery, 
    ...
    dc.title company, dc.isfree, dc.login, dc.updated_at updated_at, dc.site site, dc.image
    FROM company_item ci INNER JOIN data_company dc ON dc.id=ci.company_id $q1
    WHERE dc.status AND ci.price>0 AND ci.item_id=$model->id $q2 ORDER BY dc.isfree, dc.weight, stats DESC");
